...
async function setTournament(db: PostgresJsDatabase) {
  ...
  .values({
    ...
    banOrder: 'ABBAAB',
    concludesOn: new Date(1681106400),
...
async function setPrizes(db: PostgresJsDatabase, tournamentId: number) {
  let [firstPlacePrize, secondPlacePrize, thirdPlacePrize] = await db
    ...
    .values([{
      ...
      trophy: false,
      type: 'tournament',
      additionalItems: ['5WC 2023 Themed Momokai Tap Trio', '5WC 2023 Themed Foxbox tablet cover or mouse pad'],
      placements: [1]
    }, {
      tournamentId,
      badge: false,
...
async function setStaffMembersAndRoles(db: PostgresJsDatabase, tournamentId: number, osuClient: OsuClient, discordBotToken: string) {
  let [
    _debuggerRole,
    hostRole,
    cohostRole,
    ...
    .values([
      ...
      },
      {
        ...
        name: 'Developer',
        color: 'fuchsia',
        permissions: ['view_staff_members', 'view_regs', 'view_pooled_maps', 'view_matches', 'view_stats'],
        order: 5
      },
      {
        tournamentId,
  ...
  let mqaUserIds = [14806365, 9477784, 4420014, 10224047, 8443945, 11612720, 6036351, 12016150, 12139352, 13055978, 9405745, 11310892, 12058601, 8401053, 11955929, 6843383, 7516954];
  let mqa = map(users, mqaUserIds, mqaRole.id);

  let refUserIds = [6518510, 17959882, 8589763, 10108853, 6263040, 19650134, 9402889, 16060432, 13456818, 18315188, 6675367, 4420014, 12865368, 11612720, 11730796, 4722369, 19030042, 9918921, 6814521, 13055978, 12086495, 9074108, 6471972, 11955929];
  ...
  let mapperUserIds = [19884809, 3545323, 9074780, 7535045, 689997, 5403374, 4420014, 9706291, 7856835, 4681578, 8157492, 6632605, 5099768, 6928574, 7075211, 6693266];
  let mappers = map(users, mapperUserIds, mapperRole.id);

  let replayerUserIds = [654296, 14806365, 4420014, 10224047, 8150535, 8443945, 12904237, 16287466, 12016150, 12139352, 6221425, 10976903, 9405745, 11310892, 9047729, 12058601, 8191845, 9074108, 8401053, 6471972, 6843383, 11234706, 7516954];
  let replayers = map(users, replayerUserIds, replayerRole.id);

  let streamerUserIds = [13175102, 16060432, 6675367, 4420014, 14309415, 4722369, 2916205, 6221425, 18087655, 12455868, 10976903];
  let streamers = map(users, streamerUserIds, streamerRole.id);

  let commUserIds = [6518510, 3942633, 13175102, 13141032, 6968364, 4681578, 8196548, 9453134, 14309415, 4722369, 6036351, 12139352, 6221425, 14551370, 8191845, 6471972, 11955929, 6693266, 11658236];
  ...
  let removedDuplicateMembers = [...new Map(insertStaffMembers.map(member => [member.userId, member])).values()];
  let staffMembers = await db
    .insert(dbStaffMember)
    .values(removedDuplicateMembers)
    .returning({
      id: dbStaffMember.id
    });
...
export async function seedRealTournament(db: PostgresJsDatabase, osuClient: OsuClient, discordBotToken: string, storageEndpoint: string, storagePassword: string) {
