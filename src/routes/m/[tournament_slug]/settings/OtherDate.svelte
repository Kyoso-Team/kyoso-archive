...
<div class="card grid grid-cols-[1fr_auto] gap-4 w-full bg-surface-200-700-token p-4">
  <div>
    ...
    <span class="block text-surface-600-300-token text-sm">
      {date.onlyDate
        ? formatDate(fromDate, 'shortened')
        : `${formatDate(fromDate, 'shortened')}, ${formatTime(fromDate)}`}
      {toDate
        ? date.onlyDate
          ? ` - ${formatDate(toDate, 'shortened')}`
          : ` - ${formatDate(toDate, 'shortened')}, ${formatTime(toDate)}`
        : ''}
    </span>
  </div>
