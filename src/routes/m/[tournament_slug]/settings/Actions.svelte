...
<div class="grid grid-cols-[auto_auto] gap-2 mt-4">
  <div class="flex gap-2 max-2md:col-span-2">
    <button class="btn variant-filled-error hidden 2md:block" disabled={!hasUpdated} on:click={onReset}>
      Reset
    </button>
    {#if hasUpdated}
      <div class="card variant-soft-warning flex justify-center items-center py-[9px] px-5 w-full 2md:w-max" transition:fade={{ duration: 150 }}>
  ...
  <div class="flex justify-end w-full gap-2">
    {#if disableAddBtn !== undefined && onAdd}
      <button class="btn btn-sm variant-filled" disabled={disableAddBtn} on:click={onAdd}>Add</button>
    {/if}
    <button class="btn variant-filled-primary" disabled={disableUpdateBtn} on:click={onUpdate}>Update</button>
  </div>
</div>
