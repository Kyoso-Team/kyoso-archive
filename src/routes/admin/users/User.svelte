<script lang="ts">
  ...
  $: showRemoveBtn = hovering && type !== 'owner' && type !== 'banned' && (isCurrentUserTheOwner || !isCurrentUserTheOwner && type !== 'admin');
...
<div class="flex items-center p-2 gap-2 card w-72" role="presentation" on:mouseenter={onMouseEnter} on:mouseleave={onMouseLeave}>
  ...
  <div class="overflow-hidden w-40">
    ...
    <div class="flex gap-1 items-center">
      ...
      <a href={`https://discordapp.com/users/${user.discord.discordUserId}`} class="truncate text-sm hover:opacity-75 duration-150 w-max">{user.discord.username}</a>
    </div>
  </div>
  {#if showRemoveBtn || showSearchBtn}
    <div class="w-[43px] h-[43px] relative" transition:scale={{ duration: 150, start: 0.5, opacity: 0 }}>
      {#if showSearchBtn}
        <button class="btn-icon variant-filled absolute right-0 top-0" transition:scale={{ duration: 150, start: 0.5, opacity: 0 }} on:click={onLookupUser}>
          <Search size={24} />
        </button>
      {:else if showRemoveBtn}
        <button class="btn-icon variant-filled-error absolute right-0 top-0" transition:scale={{ duration: 150, start: 0.5, opacity: 0 }} on:click={onAction}>
          <X size={24} />
        </button>
      {/if}
    </div>
