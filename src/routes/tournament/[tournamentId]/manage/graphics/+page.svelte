...
<script lang="ts">
  ...
  let currentBanner: string | undefined = 'http://127.0.0.1:5173/uploads/tournament-banners/00001.png';
...
<div class="center-content">
  <h1>Graphics</h1>
  <div class="my-4">
    <h2 class="text-center pb-2">Banner</h2>
    <span>Current banner:</span>
    <div class="card w-80 aspect-[21/9] overflow-hidden relative border border-primary-600">
      {#if newBanner || currentBanner}
        <img
          src={newBanner || currentBanner}
          alt="banner"
          class="w-full h-auto absolute -inset-full m-auto"
        />
      {/if}
    </div>
    <div>
      ...
      >Undo Change</button>
      <FileButton name="banner-file" accept=".png, .jpg, .jpeg, .gif" bind:files={bannerFile}>Choose File</FileButton>
    </div>
  </div>
</div>