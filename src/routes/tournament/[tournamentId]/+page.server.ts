...
export const load = (async (event) => {
  ...
  let prizesQuery = db
    .select({
      ...
      cash: select(dbPrizeCash, ['value', 'metric', 'currency'])
    })
    .from(dbPrize)
  ...
  let staffMembersQuery = db
    .select({
      role: select(dbStaffRole, ['name', 'color']),
      user: select(dbUser, ['id', 'osuUserId', 'osuUsername']),
