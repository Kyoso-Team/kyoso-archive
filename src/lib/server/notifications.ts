...
import { past } from './sql';
import type { SQL } from 'drizzle-orm';
import type { AnyPgColumn, AnyPgSelect, AnyPgTable, PgTransaction } from 'drizzle-orm/pg-core';
import type postgres from 'postgres';
import type { notificationLinkTypes } from '$lib/constants';
import type { AnyPgNumberColumn } from '$lib/types';
...
>(notifications: T2): Promise<T2> {
  ...
  const ids = Object.fromEntries(vars.map((v) => [v, []] as any)) as Record<
    (typeof vars)[number],
    string[]
  >;
